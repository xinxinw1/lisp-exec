...
/* require tools >= 3.1 */
/* require lisp-tools */
/* require lisp-parse */ // exe(a) runs prs(a)
...
(function (win, udf){
  ...
  function eqq(a, env, lvl){
    ...
    var arr = eqq2(car(a), env, lvl);
    return car(arr)(cdr(arr), eqq(cdr(a), env, lvl));
  }
  ...
  function eqq2(a, env, lvl){
    ...
    switch (car(a)){
      case "uq":
        ...
        if (car(ret) == cons)return cons(car(ret), lis("uq", cdr(ret)));
        return ret;
      case "uqs":
        if (lvl == 1)return cons(app, evl(cadr(a), env));
    }
    return cons(cons, eqq(a, env, lvl));
  }
  ...
  function get(a, env){
    if (udfp(env)){
      if (is(a, "nil"))return [];
      if (has(/^c[ad]+r$/, a))return cxr(a);
      err(get, "Unknown variable a = $1", a);
    }
    if (udfp(env[a]))return get(a, env[0]);
    return env[a];
  }
